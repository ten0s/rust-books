...
/// Parse the string `s` as a coordinate pair, like `"400x600"` as `"1.0,0.5"`.
///
/// Specifically, `s` should have the form <left><sep><right>, where <sep> is
/// the character give the by the `separator` argument, and <left> and <right> are both
/// strings that can be parsed by `T::from_str`.
///
/// If `s` has the proper form, return `Some<(x, y)>`. It it doesn't parse
...
/// arguments specify points on the complex place corresponding to the upper-left
